...
### 인증된 사용자(메인)
3. **상단 헤더**: 로고(chord_ai) + 심플한 네비 + 우측 사용자 아바타(이메일 첫 글자) → 클릭 시 메뉴(로그아웃)
4. **메인 영역**: 음원 파일 드래그앤드롭 업로드 영역
5. **결과 영역**: 추출된 코드 진행 표시 (마디별 칩 형태)
6. **채팅 영역**: LLM과 대화하는 인터페이스
7. **출력 영역**: 악보 미리보기(OpenSheetMusicDisplay) + 오디오 플레이어
