...
## 13. 프로젝트 방향 전환 — 코드 추출 메인 → 직접 입력 메인 + 추출 보조

**원래 계획**
PRD대로 "MP3 업로드 → 자동 코드 추출 → LLM 편곡 → 악보/오디오 생성"을 메인 흐름으로 운영. 사용자는 음원만 던지면 된다는 그림.

**발생한 문제**
- 코드 추출 정확도 한계가 누적적으로 드러남(이슈 2/8/9/10):
  - sus 코드 chroma 검출 실패 — 두 곡 연속 0개(이슈 9)
  - 9th/11th 같은 텐션은 chordino도 안정 검출 못 함
  - CM7 ↔ Em7, Bm7 ↔ D 같은 상부구조 모호성 잔존(이슈 10) — 베이스 정정으로 일부만 회복
- LLM 편곡도 자유 텍스트 입력으로는 결과 비결정적·재현 불가:
  - "재즈 느낌" 같은 모호한 요청 → 호출마다 결과 다름, 회귀 테스트 불가
  - 잘못된 코드 표기, 키 이탈, music21 미지원 표기(이슈 11)가 silent fail로 이어짐
- 두 불확실성이 곱해지니 사용자 체감 품질을 보장할 수 없음.

**원인**
- 음원 → 코드 추출은 신호처리·모델 표현력의 본질적 한계가 있음. 알고리즘을 바꿔도 일정 수준 이상 안 올라감.
- 자유 텍스트 → LLM은 검증 가능한 시스템이 아님(테스트·재현·디버깅 불가).
- 정확도 99%를 기대하는 사용자에게 70~80%를 주는 메인 기능은 신뢰를 깨뜨림.

**해결 (방향 전환)**
- **메인 입력을 "사용자 직접 코드 입력"으로 변경** — 정확도 책임을 사용자에게 (사용자는 청음·검색으로 코드를 안다는 전제).
- **MP3 추출은 "코드를 모를 때 쓰는 보조 진입점"으로 강등** + 정확도 한계를 UI에서 명시(sus/9th 미검출, 상부구조 모호성).
- **LLM 입력을 구조화** — 장르/복잡도/텐션/베이스/리듬 5개 옵션 + 자유 텍스트 보조. 핵심 제어는 결정론적으로.
- **LLM 출력을 structured JSON + 룰 베이스 검증 레이어** — chord_postprocess의 normalize_label·다이어토닉 보정을 LLM 출력에도 적용해 silent fail 방지.
- **MIDI 미리듣기로 "생성 → 청음 → 수정" 루프 단축** — 피아노+베이스 2트랙으로 MVP.
- **편곡 단위 = 8마디/섹션** — 곡 전체 한 번이 아닌 분할로 토큰·일관성·캐싱 동시 해결.

자세한 신규 설계는 [PRD.md](PRD.md) 참고.

**배운 점**
- 사용자가 "이건 정확도 99%여야 한다"고 기대하는 영역에서 알고리즘이 그걸 못 주면, **기능을 줄여서 정확도를 빼는 게 아니라 정확도 책임을 사용자에게 넘기는** 설계 변경이 더 솔직함.
- 자유 텍스트 LLM은 데모에는 좋지만 production 도구로는 검증 불가. **structured input/output + 룰 검증**이 production-grade의 최소 조건.
- 이미 만든 추출 파이프라인을 버리지 말고 보조 진입점으로 retain — "쓸모는 있되 무대 중앙은 아님" 포지션.
- 방향 전환이 늦어질수록 손실이 커짐. 알고리즘 한계가 명확해진 시점(이슈 9·10)이 사실상 신호였음.
...
13. **알고리즘 한계가 메인 기능 신뢰도를 깬다** → "기능을 줄여 정확도 빼기"가 아니라 "정확도 책임을 사용자에게 넘기기"로 설계 자체를 바꾼다. + 자유 텍스트 LLM은 production 도구 불가 → structured I/O + 룰 검증이 최소 조건.
